...
[ PrintKysNomini i from k poikkeus;
	...
	!    "n‰yt‰ mit‰ kenelle?"
	!           1    2
	! kysymykset "k‰‰nteisess‰" j‰rjestyksess‰
	!    "n‰yt‰ kenelle mit‰?" 
	!           2       1      
	if (action_reversed) poikkeus = true;
	! jotkut verbit ovat jo valmiiksi "toisinp‰in",
     	! esim ##ask:
	!    "kysy kenelt‰ mit‰?"
	!          (1)     (2)
	! jolloin ##ask aiheuttaa "poikkeuksen" silloin
	! kun action_reversed on ep‰tosi.
	! (t‰m‰ on sekavaa, voisi tehd‰ niin ett‰
	! l‰htˆkohtaisesti 0 verbi - 1 subjekti - 2 objekti?
    ! jolloin oletus olisi aina esim. "n‰yt‰ kenelle mit‰?")
    !
    ! ts. sallitaan 1. nominin taivutus "N‰yt‰ Ristolle mit‰?"
	if (action_to_be == ##Ask or ##AskFor && action_reversed == false)
	    poikkeus = true;

	! Tulostaa v‰‰r‰n sijan (partitiivin koska olettaa sen v‰‰rin
	! tulevan annettavasta objektista eik‰ subj.)
	!   >anna ristolle
	!   anna Ristoa mit‰?
	!
	! Eik‰ t‰t‰ n‰in ratkaista:
...
[ PrintKysymys verbi from k obj kys _kys;
   ! print "! from ", from, ", k ", k, ", etype ", etype, "^";
	...
	kys = "mik‰";
    ! kysy kenelt‰ mit‰ -- poikkeus (on s‰‰ntˆ? --> tee siit‰?)
    if (action_to_be == ##Ask or ##Ask) kys = "kenelt‰";
    ...
    ! kysy ristolta mit‰
    if (action_to_be == ##Ask or ##AskFor) _kys = "mit‰";
...
!	kys = obj.kys;
	...
	print " ";
	! jos toinen kysymys, eik‰ k‰‰nteinen, tulosta _kys,
