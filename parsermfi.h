...
! ...Pit‰isi katsoa mihin ao. rutiineista on j‰‰nyt oleellisia muutoksia
! (verrattuna parserm-tiedostoon) ja j‰tt‰‰ vain muokatut.
...
[ Parser__parse  results   syntax line num_lines line_address i j k
  ...
  .ReParse;
    ...
    LanguageToInformese(l); !! lis‰tty l (reparse vai ei?)
    #IfV5;
    ! Re-tokenise:
    Tokenise__(buffer,parse);
    #Endif; ! V5
  ...
  .VerbAccepted;
    ...
    #Endif; ! DEBUG
	!!!§§§ print "[C best stuck nextbest stuck]";
  ...
  ! if (suffix == true) {suffix = false; etype = STUCK_PE;} else !!!§§§§ very temp whole line	
    etype = best_etype;
    ...
    !!!##
    !!!## if (temporary == true && etype == CANTSEE_PE) {temporary = 0;
    !!!##  
    !!!## #Ifdef DEBUG;				!!!§§§
    !!!## if (parser_trace > 0) print "!temporary flag reseted to 0 & CANTSEE_PE diverted to STUCK_PE!^"; 		!!!§§§
    !!!## #Endif;					!!!§§§ ">poistu tuolissa -> ("Et n‰e mit‰‰n sellaista" => "En k‰sitt‰nyt tuota..." 
    !!!##
    !!!## etype = STUCK_PE; } 			!!!!§§§§
...
[ ParseToken given_ttype given_tdata token_n x y;
 ...
 !#Ifdef HUONO_SUOMI;
 !#Ifdef DEBUG;
 ! if (parser_trace >= 1 && y == GPR_REPARSE) print "   [HUONO_SUOMI (ParseToken) hypp‰‰ ReParse!]^";
 !#Endif; ! DEBUG
...
[ ParseToken__ given_ttype given_tdata token_n
  ...
  .TryAgain;
    ...
    l = Descriptors();
    if (l ~= 0) {!!!§§§ { print "[Q etype ~l~]"; 
    etype = l; return GPR_FAIL; 
    }
  ...
  .TryAgain2;
    ...
    if (token ~= HELD_TOKEN) {
        ...
        if (token_allows_multiple && ~~multiflag) {
            if (best_etype==MULTI_PE)  !!!§§§ {print "[P best stuck]"; 
            best_etype=STUCK_PE;
            !!!§§§}
            multiflag = true; 
        }
        if (l == 0) {
            ...
            if (etype == MULTI_PE or TOOFEW_PE && multiflag) !!!§§§{print "[O etype stuck]"; 
            etype = STUCK_PE;
            !!!§§§}
            !etype=STUCK_PE; !§
       
            else !!!§§§{print "[N etype cantsee()]"; 
            etype=CantSee();
            !!!§§§} !§?
            jump FailToken;
        } ! Choose best error
...
!*** TAITAA BUGATA ("***") ????
    if ((indef_wanted > 0 || prev_indef_wanted > 0) && (~~multiflag))!!!§§§ {print "[I etype multi]"; 
    etype = MULTI_PE; !!!§§§}
...
!!!## kierros2 = 1;
!!!##	#Ifdef DEBUG;				!!!§§§
!!!##	if (parser_trace > 0) print "^<ND (at .WhichOne): Kierros2 set TRUE!>^"; 	!!!§§§
!!!##	#Endif;					!!!§§§ 
...
!!!# (parserm) ! Constant SCORE__CHOOSEOBJ = 1000;
!!!# (parserm) ! Constant SCORE__IFGOOD = 500;
!!!# (parserm) ! Constant SCORE__UNCONCEALED = 100;
!!!# (parserm) ! Constant SCORE__BESTLOC = 60;
!!!# (parserm) ! Constant SCORE__NEXTBESTLOC = 40;
!!!# (parserm) ! Constant SCORE__NOTCOMPASS = 20;
!!!# (parserm) ! Constant SCORE__NOTSCENERY = 10;
!!!# (parserm) ! Constant SCORE__NOTACTOR = 5;
!!!# (parserm) ! Constant SCORE__GNA = 1;
!!!# (parserm) ! Constant SCORE__DIVISOR = 20;
...
#Ifdef DEBUG;
	if (parser_trace >= 1) 
print "   [HUONO_SUOMI (Parsermfi: Adjudicate) Unable to choose best... mutta ‰l‰ kysy pelaajalta.]^";
#Endif; ! DEBUG
! antaa ao. virheilmoituksen (puklu: parsererror tulostaa sen), ei uudelleenparsimispyyntˆ‰.
...
!!! muokattu
             	...
             	if (CaseIs ~= 0 && k >= 2) ! ensimm‰inen sana tulostuu nom. seuraavat taipuu			
             		switch (CaseIs) {
...
[ CantSee  i w e;
    ...
    if (scope_token ~= 0) {
  	...
  	#Ifdef DEBUG;			
    	if (parser_trace > 0) print "^<CANTSEE: scope error!>^"; 
	#Endif;				
...
e = CANTSEE_PE;
   ...
   ! else if (Refers(i,wn) == 1) e = TOOLIT_PE;

	i = actor; while (parent(i) ~= 0) i = parent(i); 
    wn--;
 !   if (Refers(i,wn) == 1) e = STUCK_PE;
